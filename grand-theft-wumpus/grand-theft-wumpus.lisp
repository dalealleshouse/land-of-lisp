...
; random returns a random natrual number less than the passed in number
; 1+ so we can number the nodes 1-30 instead of 0-29
(defun random-node ()
  (1+ (random *node-num*)))
...
; collect works kinda like yeild in C#
; append takes all items in the supplied list and flattens them into a single list
...
; if any data is found edge-alist, there is a cop there because it's the only data that can be there
...
; mapcan like mapcar but assumes that the values generated by the mapping function are all lists that should be appended together
